...
func TestDistributedLock(t *testing.T) {
	...
	// Test with expired TTL - use a different key to avoid conflicts
	mutex4, err := cache.Lock("test-key-expire", 10*time.Millisecond)
	assert.NoError(t, err)
	assert.NotNil(t, mutex4)
	// Unlock mutex4 first
	err = cache.Unlock(mutex4)
	assert.NoError(t, err)
	time.Sleep(20 * time.Millisecond) // Wait a bit

	// Should be able to acquire lock after unlock
	mutex5, err := cache.Lock("test-key-expire", 10*time.Second)
	assert.NoError(t, err)
