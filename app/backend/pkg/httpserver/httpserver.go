...
func (s *HTTPServer) AddRouteV2(group *gin.RouterGroup, cfg RouteConfig) {
	...
	if group == nil {
		...
		s.logger.Info(context.Background(), "Route initialized", "Method", cfg.Method, "Path", cfg.Path, "Description", cfg.Description, "Params", params)
	} else {
		group.Handle(cfg.Method, cfg.Path, handler)
		s.logger.Info(context.Background(), "Route initialized", "Method", cfg.Method, "Path", joinPaths, "Description", cfg.Description, "Params", params)
	}
}
...
func (s *HTTPServer) Start() error {
	...
	s.logger.Info(ctx, fmt.Sprintf("Starting HTTP server on address: %v...", addr))
	if err := s.router.Run(addr); err != nil {
		s.logger.Error(ctx, fmt.Sprintf("Failed to start HTTP server: %s", err))
		return err
	}
