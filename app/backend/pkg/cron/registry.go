...
func (jr *JobRegistry) RegisterJob(name string, factory JobFactory) {
	...
	jr.logger.Info(context.Background(), "Registered job factory", "job", name)
}
...
func (jr *JobRegistry) LoadConfigFromFile(configPath string) error {
	...
	for _, config := range configs {
		...
		jr.logger.Info(context.Background(), "Loaded job config", "job", config.Name, "schedule", config.Schedule, "enabled", config.Enabled)
	}
...
func (jr *JobRegistry) RegisterJobsWithManager(manager *CronManager) error {
	...
	for name, config := range jr.configs {
		if !config.Enabled {
			jr.logger.Info(ctx, "Skipping disabled job", "job", name)
			continue
		}

		job, err := jr.CreateJob(name)
		if err != nil {
			jr.logger.Error(ctx, "Failed to create job", "job", name, "error", err.Error())
			continue
		}

		if err := manager.AddJob(config.Schedule, job); err != nil {
			jr.logger.Error(ctx, "Failed to add job to manager", "job", name, "error", err.Error())
			continue
		}

		jr.logger.Info(ctx, "Successfully registered job with manager", "job", name)
	}
...
func (jrb *JobRegistryBuilder) WithConfigFile(configPath string) *JobRegistryBuilder {
	if err := jrb.registry.LoadConfigFromFile(configPath); err != nil {
		jrb.registry.logger.Error(context.Background(), "Failed to load config file", "path", configPath, "error", err.Error())
	}
	return jrb
}
...
func NewDefaultJobRegistry(logger logger.ILogger) *DefaultJobRegistry {
	...
	registry.RegisterJob("heartbeat", func() Job {
		return NewFuncJob("heartbeat", func(ctx context.Context) error {
			registry.logger.Info(ctx, "Heartbeat job executed")
			return nil
		})
	})
	...
	registry.RegisterJob("cleanup", func() Job {
		return NewFuncJob("cleanup", func(ctx context.Context) error {
			registry.logger.Info(ctx, "Cleanup job executed")
