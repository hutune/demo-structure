...
func (cm *CronManager) AddJob(schedule string, job Job) error {
	...
	if entryID, exists := cm.jobs[jobName]; exists {
		...
		cm.logger.Info(context.Background(), "Removed existing cron job", "job", jobName)
	}
	...
	if err != nil {
		cm.logger.Error(context.Background(), "Failed to add cron job", "job", jobName, "error", err.Error())
		return err
	}

	cm.jobs[jobName] = entryID
	cm.logger.Info(context.Background(), "Added cron job", "job", jobName, "schedule", schedule)
	return nil
}
...
func (cm *CronManager) executeJob(job Job) {
	...
	cm.logger.Info(ctx, "Starting cron job", "job", jobName)
	startTime := time.Now()
	...
	if info, exists := cm.jobInfo[jobName]; exists {
		...
		if err != nil {
			...
			info.FailCount++
			cm.logger.Error(ctx, "Cron job failed", "job", jobName, "error", err.Error(), "duration", duration)
		} else {
			info.Status = JobStatusSuccess
			info.LastError = ""
			cm.logger.Info(ctx, "Cron job completed successfully", "job", jobName, "duration", duration)
		}
	}
	cm.jobsMu.Unlock()
}
...
func (cm *CronManager) Start() {
	...
	if cm.isRunning {
		cm.logger.Warn(context.Background(), "Cron manager is already running")
		return
	}

	cm.cron.Start()
	cm.isRunning = true
	cm.logger.Info(context.Background(), "Cron manager started", "jobs_count", len(cm.jobs))
}
...
func (cm *CronManager) Stop() {
	...
	if !cm.isRunning {
		cm.logger.Warn(context.Background(), "Cron manager is not running")
		return
	}
	...
	select {
	case <-ctx.Done():
		cm.logger.Info(context.Background(), "Cron manager stopped gracefully")
	default:
		cm.logger.Info(context.Background(), "Cron manager stopped")
	}
...
func (cm *CronManager) RemoveJob(jobName string) {
	...
	if entryID, exists := cm.jobs[jobName]; exists {
		...
		cm.logger.Info(context.Background(), "Removed cron job", "job", jobName)
	}
}
