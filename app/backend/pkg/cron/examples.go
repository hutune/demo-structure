...
func (d *DatabaseCleanupJob) Run(ctx context.Context) error {
	d.logger.Info(ctx, "Starting database cleanup", "retention_days", d.retentionDays)
	...
	d.logger.Info(ctx, "Cleaning up records", "cutoff_date", cutoffDate.Format("2006-01-02"))
	...
	d.logger.Info(ctx, "Database cleanup completed successfully")
	return nil
}
...
func (c *CacheWarmupJob) Run(ctx context.Context) error {
	c.logger.Info(ctx, "Starting cache warmup", "keys_count", len(c.cacheKeys))
	...
	for _, key := range c.cacheKeys {
		...
		default:
			...
			c.logger.Debug(ctx, "Warming up cache key", "key", key)
			time.Sleep(10 * time.Millisecond)
		}
	}

	c.logger.Info(ctx, "Cache warmup completed successfully")
	return nil
}
...
func (h *HealthCheckJob) Run(ctx context.Context) error {
	h.logger.Info(ctx, "Starting health check", "endpoints_count", len(h.endpoints))
	...
	for _, endpoint := range h.endpoints {
		...
		default:
			...
			h.logger.Debug(ctx, "Checking endpoint", "endpoint", endpoint)
	...
	h.logger.Info(ctx, "Health check completed successfully")
	return nil
}
...
func (r *ReportGenerationJob) Run(ctx context.Context) error {
	r.logger.Info(ctx, "Starting report generation", "type", r.reportType, "output", r.outputPath)
	...
	for i, step := range steps {
		...
		case <-ctx.Done():
			return ctx.Err()
		default:
			r.logger.Info(ctx, "Report generation step", "step", i+1, "description", step)
	...
	r.logger.Info(ctx, "Report generation completed successfully", "output_file", r.outputPath)
	return nil
}
...
func SetupExampleJobs(logger logger.ILogger) (*CronManager, *JobRegistry) {
	...
	if err := registry.RegisterJobsWithManager(manager); err != nil {
		logger.Error(context.Background(), "Failed to register jobs with manager", "error", err.Error())
	}
